...
웹 문서를 작성할 때는 기본적으로 **헤더 영역(`header`)과 콘텐츠 영역(`main`), 그리고 푸터 영역(`footer`)의 3단 구조**로 화면을 분할합니다.

* 헤더 영역(`header`): 웹 페이지 전체에 대한 제목(`h1`)과 웹 페이지 전체에 대한 글로벌 네비게이션(`nav`)을 포함하는 영역입니다. 이때 빼먹기 쉬운 한 가지 주의할 점은 `nav` 내부에 반드시 적절한 제목 요소(`h1` ~ `h6`)를 포함해야 한다는 점입니다.
* 콘텐츠 영역(`main`): 웹 페이지에 존재하는 메인 콘텐츠를 포함하는 영역입니다. 각각의 연관된 콘텐츠는 `section` 또는 `article`로 구분하고, 구분된 각 영역은 **반드시 적절한 제목 요소를 포함해야 합니다.** 일반적으로 각 섹션은 `section` 요소를 사용하여 구분하되, 뉴스 기사, 블로그 포스트, 트윗와 같이 독립적으로 재배포할 가치가 있는 콘텐츠인 경우에 `article`로 처리하도록 합니다. `section`과 `article` 중 무엇을 써야 할지 헷갈린다면 그냥 `section`만 사용해도 괜찮습니다.
* `footer`: 웹 사이트에 대한 부가 정보(회사 또는 저자 정보), 저작권 정보를 포함하는 영역입니다.

> 이 세 가지 영역 외에 추가적으로 `aside` 영역을 포함할 수 있습니다. `aside`는 광고 또는 사이드바와 같이, 본문에서 삭제되더라도 전체 콘텐츠에 영향을 미치지 않는 영역을 마크업할 때 사용됩니다.

이에 따라 LINE 웹페이지의 구조를 잡아보면 다음과 같이 나눌 수 있을 것입니다.
